...
import { isBuild, isWindows, page, testDir } from './unit'
...
export const expectWithRetry = <T>(
  getActual: () => Promise<T>,
): Promisify<Assertion<T>> => {
  return new Proxy(
    {},
    {
      get(_target, key) {
        return async (...args: any) => {
          await withRetry(async () => {
            ...
            const method = (expect(actual) as any)[key]
